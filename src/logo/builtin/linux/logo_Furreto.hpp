#pragma once
#include <vector>
#include <string>

namespace rfxh::logo::builtin::cat_linux {

inline std::vector<std::string> logo_Furreto() {
    return {
        "           .xOOko      .odd,",
        "          oXWWKOOOO. 'ONWW0kkk.",
        "         .k0XKOOOOOOcOONWNOOOOO.",
        "          xOOOOOOOOOkkOOOOOOOOO;",
        "   .O0OkkocxO000000kcdk0OO0OOkx",
        "   kWMXkkkkloxkkkx;  :dxxxxddc...",
        "  'kO0OOOOOkc          .cl:..kk0KK0Okc",
        "  ;kOOO0000xd.  dO00000Oo  .xkONMMXOOOO",
        ".dddxkOOOkddc.kKNWWN000000l.ddk0000OOOO.",
        " 'dd:;ddddd;.dKMMMWK00KKK0O::ddxkO00Oko",
        "         .okxkOKK0kkOO00KKOxxlodddddddl",
        "       .00OOkkkkkkkkOOO00OOOO0O;  .dddl",
        "      'kkkkkxxkkkkkkkOOkxdxkxxddd.",
        "      cddddddddxxkkkkkxddddddddddo",
        "      'ddddddodddddddddddddddddddc",
        "       .ddddddodddddddddodddddddc",
        "                .odddo.",
        "",
        "               .kOOkkk;",
        "              lkKWNkkkxc",
        "             kkxkkkkkkx.",
        "              ,,..xxx.",
    };
}

}
